...
export default function UserDashboard() {
  ...
  const dashboardData = useMemo(() => {
    ...
    const prepaidBalance = realBrl + realUsd; // Saldo pré-pago são as recargas feitas
    ...
    >
      <ResponsiveContainer mobileOptimized={true} padding={isMobile ? 'sm' : 'md'}>
        <div className={`${isMobile ? 'pt-2 space-y-6' : 'pt-4 space-y-8'}`}>
            ...
            >
              ...
              <div>
                ...
                <p className={`text-green-300 ${isMobile ? 'text-xs' : 'text-sm'} mt-1`}>
                  {language === 'pt' ? 'Saldo de recargas' : 'Recharge balance'}
                </p>
              </div>
            </motion.div>
          ...
          {/* Botões de Ação - Apenas 2 botões: Usar Cupom e Recarga */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex flex-col items-center space-y-3">
                ...
                <div className="text-center">
                  <p className="text-green-400 font-bold text-sm mb-1">
                    {language === 'pt' ? 'Recarregar Créditos' : 'Recharge Credits'}
                  </p>
                  <p className="text-gray-400 text-xs">
