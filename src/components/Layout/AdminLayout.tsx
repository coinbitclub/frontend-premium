...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children, title = 'Admin Dashboard' }) => {
  ...
  const navigation = [
    ...
    { name: 'Relatórios', href: '/admin/reports', icon: FiFileText },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900/95 backdrop-blur-sm border-r border-gray-800 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0`}>
          ...
          <div className="space-y-1">
            {navigation.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon className={`mr-3 h-5 w-5 ${isActive ? 'text-yellow-400' : 'text-gray-400'}`} />
                  {item.name}
                </Link>
              );
            })}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-800">
          ...
          <button
            ...
            className="w-full flex items-center px-3 py-2 text-sm text-gray-300 rounded-lg hover:bg-gray-800 hover:text-white transition-colors"
          >
        ...
        <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-800 bg-gray-900/95 backdrop-blur-sm px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-1 gap-x-4 lg:gap-x-6">
            <div className="relative flex flex-1">
              ...
              <input
                className="block h-full w-full border-0 bg-gray-800/50 py-0 pl-10 pr-3 text-white placeholder:text-gray-400 focus:ring-2 focus:ring-yellow-400 sm:text-sm rounded-lg"
                placeholder="Buscar..."
                type="search"
              />
            </div>
        ...
        <main className="py-6">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-6">
              ...
              <p className="text-gray-400">Painel administrativo completo</p>
            </div>
            {children}
          </div>
        </main>
