...
import { FiDollarSign, FiTrendingUp, FiTarget, FiRefreshCw, FiAlertCircle, FiArrowRight, FiUsers, FiGift, FiCreditCard, FiTag } from 'react-icons/fi';
...
export default function UserDashboard() {
  ...
  // Função para lidar com recarga
  const handleRecharge = useCallback(async () => {
    if (!rechargeAmount || parseFloat(rechargeAmount) < 20) {
      showToast(
        language === 'pt' ? 'Valor mínimo para recarga: R$ 20,00' : 'Minimum recharge amount: $20.00',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        showToast(
          data.error || (language === 'pt' ? 'Erro ao processar recarga' : 'Error processing recharge'),
          'error'
        );
      }
    } catch (error) {
  ...
  const handleCoupon = useCallback(async () => {
    ...
    try {
      setProcessing(true);
      // Simular aplicação de cupom - aqui você conectaria com sua API real
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      showToast(
        language === 'pt' ? `Cupom ${couponCode} aplicado com sucesso!` : `Coupon ${couponCode} applied successfully!`,
        'success'
      );
      setShowCouponModal(false);
      setCouponCode('');
      ...
      showToast(
        language === 'pt' ? 'Cupom inválido ou expirado' : 'Invalid or expired coupon',
        'error'
      );
    } finally {
      setProcessing(false);
      ...
      <ResponsiveContainer mobileOptimized={true} padding={isMobile ? 'sm' : 'md'}>
        <div className={`${isMobile ? 'pt-2 space-y-6' : 'pt-4 space-y-8'}`}>
          ...
          <div className={`grid ${isMobile ? 'grid-cols-2 gap-3' : 'grid-cols-4 gap-4'}`}>
            ...
            >
              ...
              <div>
                ...
                <p className={`text-white font-bold ${isMobile ? 'text-lg' : 'text-2xl'}`}>
                  {dataLoading ? '...' : `$${(dashboardData.balances.prepaid * 0.1).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                </p>
                <p className={`text-purple-300 ${isMobile ? 'text-xs' : 'text-sm'} mt-1`}>
                  {language === 'pt' ? 'Bônus e promoções' : 'Bonuses & promotions'}
                </p>
              </div>
            </motion.div>
          ...
          >
              ...
              <div className="flex flex-col items-center space-y-3">
                ...
                <div className="text-center">
                  <p className="text-green-400 font-bold text-sm mb-1">
                    {language === 'pt' ? 'Recarga de Créditos' : 'Recharge Credits'}
                  </p>
            ...
            {/* Botão Trading (existente) */}
            <button
              onClick={() => router.push('/user/operations')}
              ...
              <div className="flex flex-col items-center space-y-3">
                ...
                <div className="text-center">
                  <p className="text-blue-400 font-bold text-sm mb-1">
                    {language === 'pt' ? 'Iniciar Trading' : 'Start Trading'}
                  </p>
                  <p className="text-gray-400 text-xs">
                    {language === 'pt' ? 'Operar agora' : 'Trade now'}
                  </p>
                </div>
              </div>
            </button>
